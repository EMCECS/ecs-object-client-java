...
public class S3SignerV4 extends S3Signer {
    ...
    @Override
    public void sign(ClientRequestContext requestContext, String resource, Map<String, String> parameters, Map<String, List<Object>> headers) {
        // # Preparation, add x-amz-date and host headers
        String date;
        String serviceType = getServiceType();
        date = getDate(parameters, headers);
        String shortDate = getShortDate(date);
        addHeadersForV4(requestContext.getUri(), date, headers);

        // #1 Create a canonical request for Signature Version 4
        String canonicalRequest = getCanonicalRequest(requestContext.getMethod(), requestContext.getUri(), parameters, headers, false);

        // #2 Create a string to sign for Signature Version 4
        String stringToSign = getStringToSign(requestContext.getMethod(), resource, parameters, headers, date, serviceType, canonicalRequest);
